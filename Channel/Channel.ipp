...
namespace sharp {
    ...
    void Channel<Type>::enqueue_element(Args&&... args) {
        // push back and element and then construct it in place
        this->queue.push_back(Channel::ValueOrException{});
        new (&this->queue.back().storage) Type{std::forward<Args>(args)...};
    }
    ...
    void Channel<Type>::send_impl(Func&& enqueue_func) {
        ...
        // check to see if the value can be sent across the channel, wait
        // until either there is a reader that can read the value or until
        // there is an open spot in the channel's buffer
        while(!this->num_readers_waiting
                || this->elements.size() == this->buffer_length) {
            this->send_cv.wait(lck);
        }
    ...
    void Channel<Type>::send(const Type& value) {
        this->send_impl([this, &]() { this->enqueue_element(value); });
    }
